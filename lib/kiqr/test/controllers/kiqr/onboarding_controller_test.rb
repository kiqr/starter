...
class Kiqr::OnboardingControllerTest < ActionDispatch::IntegrationTest
  ...
  test "can onboard user" do
    ...
    sign_in user
    post user_onboarding_path, params: { user: { personal_account_attributes: { name: "Foobar zoo" } } }
    assert_redirected_to dashboard_path
  ...
  test "validates user onboarding" do
    sign_in create(:user, personal_account: nil)
    post user_onboarding_path, params: { user: { personal_account_attributes: { name: "no" } } }
